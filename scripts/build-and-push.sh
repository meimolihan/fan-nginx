#!/bin/bash
#
# fan-nginx (Fan-Nginx) - 发布脚本（触发 GitHub Actions 单一发布流水线）
# 不在本地编译任何产物：仅更新版本号、推送代码、打 v 开头 tag。
# 推送后由 GitHub Actions 的 .github/workflows/release.yml 链式自动完成：
#   [1] Maven 编译 fan-nginx-<ver>.jar（linux amd64/arm64 通用, Java 8）
#   [2] 创建 GitHub Release 并附带 fan-nginx-<ver>.jar
#   [3] 构建推送 Docker 镜像：Docker Hub + GHCR
#   [4] 同步到 CNB 镜像仓库
#
# Usage:
#   TAG(必填) 形如 v4.4.3; --yes 免交互; -m "备注" 可选发版说明
#     bash scripts/build-and-push.sh v4.4.3 --yes -m "本次新增 xxx"
set -euo pipefail
...
[[ -z "${TAG}" ]] && error "缺少TAG参数，示例: $0 v4.4.3 --yes"
...
ensure_cnb_repo
TARGET_VER="${TAG#v}"
[[ "${TARGET_VER}" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] || error "TAG 格式错误，示例: v4.4.3"
...
info "写入发版备注 RELEASE_NOTES.md"
{
  ...
  printf '## jar 安装\n'
  printf '```bash\n'
  printf 'bash -c "$(curl -sSL https://raw.githubusercontent.com/meimolihan/fan-nginx/main/scripts/install.sh)" -p 8080 -d /var/lib/fan-nginx\n'
  printf '```\n'
  printf '\n'
  printf '```bash\n'
  printf 'docker pull mobufan/fan-nginx:latest\n'
  printf '```\n'
  printf '```bash\n'
  printf 'docker pull mobufan/fan-nginx:%s\n' "${TAG}"
  printf '```\n'
  printf '\n'
  printf '```bash\n'
  printf 'docker pull ghcr.io/meimolihan/fan-nginx:latest\n'
  printf '```\n'
  printf '```bash\n'
  printf 'docker pull ghcr.io/meimolihan/fan-nginx:%s\n' "${TAG}"
  printf '```\n'
  printf '\n'
  printf '## 二进制卸载\n'
  printf '```bash\n'
  printf 'bash -c "$(curl -sSL https://raw.githubusercontent.com/meimolihan/fan-nginx/main/scripts/uninstall.sh)" -y --purge\n'
  printf '```\n'
  printf '\n'
  printf '## CLI 命令\n'
  printf '```bash\n'
  printf 'fan-nginx status\n'
  printf 'fan-nginx credentials\n'
  printf 'fan-nginx start | stop | restart\n'
  printf '```\n'
} > RELEASE_NOTES.md
...
info "查看镜像: docker pull mobufan/fan-nginx:${TAG}"
