...
print_banner() {
  ...
  printf '%s\n' \
    "" \
    "  ${z}┌─────────────────────────────────────────┐${r}" \
    "  ${z}│${r}   ${b}fan-nginx${r}  ${l}Nginx 管理平台 · 卸载${r}      ${z}│${r}" \
    "  ${z}└─────────────────────────────────────────┘${r}" \
    ""
}
...
usage() {
  printf '%s\n' \
    ...
    "选项:" \
    "  -y, --yes        免确认，自动同意卸载" \
    "      --purge      卸载时同时删除数据目录（包含数据库、证书、配置和日志）" \
    "      --keep-data  卸载时保留数据目录" \
...
read_config() {
  ...
  while IFS='=' read -r KEY VALUE; do
    ...
    case "$KEY" in
      ...
      JAR_PATH) [ -n "$VALUE" ] && JAR_PATH="$VALUE" ;;
    esac
  done < "$CONFIG_FILE"
}

# 匹配运行中的 fan-nginx java 进程：命令行包含 jar 路径或 jar 名
find_fan_nginx_pids() {
  local args pid jar_name
  jarname="$(basename "${JAR_PATH:-}" 2>/dev/null || true)"
  while read -r pid args; do
    case " ${args} " in
      *" -jar "*)
        if { [ -n "${JAR_PATH}" ] && [[ " ${args} " == *" ${JAR_PATH} "* ]]; } \
          || { [ -n "${jarname}" ] && [[ " ${args} " == *"${jarname}"* ]]; }; then
          echo "$pid"
        fi
        ;;
    esac
  done < <(ps -eo pid=,args= 2>/dev/null | grep "java" | grep -v grep || true)
}
...
if [ -f "$SERVICE_FILE" ]; then
  [ -z "$PORT" ] && PORT=$(grep -oE -- '--server\.port=[0-9]+' "$SERVICE_FILE" | head -n1 | cut -d= -f2)
  [ -z "$PORT" ] && PORT="$DEFAULT_PORT"
  [ -z "$JAR_PATH" ] && JAR_PATH=$(grep -oE '/[^ ]*fan-nginx[^ ]*\.jar' "$SERVICE_FILE" | grep -v -- '-jar' | head -n1)
fi
...
if [ -n "$DATA_DIR" ] && [ -d "$DATA_DIR" ]; then
  ...
  if [ "$KEEP_DATA" = "1" ]; then
    ...
    read -r -p "${gl_huang}是否删除数据目录 ${DATA_DIR}？（包含数据库、证书、配置和日志）${gl_bai}[Y/n]${reset}: " DEL_DATA
    case "$DEL_DATA" in
      n|N|no|NO)
        skip "已保留数据目录 ${gl_bai}${DATA_DIR}${reset}"
        ;;
