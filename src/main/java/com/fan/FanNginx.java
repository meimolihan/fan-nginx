...
@EnableScheduling
@SolonMain
public class FanNginx {
	static Logger logger = LoggerFactory.getLogger(FanNginx.class);
	...
	public static void main(String[] args) {
		...
		Solon.start(FanNginx.class, args);
	}
	...
	private static List<String> getProcessId() {
		...
		if (SystemTool.isWindows()) {
			List<String> list = RuntimeUtil.execForLines("wmic process where \"CommandLine like '%fan-nginx%'\" get ProcessId,CommandLine");
			...
			List<String> list = RuntimeUtil.execForLines("/bin/sh", "-c", "ps -ef | grep fan-nginx");

			for (String line : list) {
				if (line.contains(".jar") && !line.contains("fan-nginx-Website")) {
					String[] lines = line.split("\\s+");
					pids.add(lines[1]);
				}
			}
	...
	private static void removeJar() {
		...
		for (File file : list) {
			if (file.getName().startsWith("fan-nginx") && file.getName().endsWith(".jar") && !file.getPath().equals(JarUtil.getCurrentFilePath())) {
				FileUtil.del(file);
